...
impl NpcServer {
    ...
    pub async fn generate_npc_response(
        ...
        let prompt = format!(
            "あなたはファンタジー世界の住人として、以下のキャラクターになりきって『一度だけ』返答してください。\n\n\
            [あなたのキャラクター設定]\n\
            - 名前: {}\n\
            - 性格・口調: {}\n\
            - 背景知識・目的: {}\n\
            ...
            - プレイヤーへの好感度: {} / 100\n\n\
            [絶対遵守のメタ・ルール]\n\
            1. あなたは絶対に対話相手（ユーザー）ではありません。あなたは「{}」本人です。\n\
            2. あなたの一人称は、キャラクターの性格に完全に一致させてください（例: ミナなら「私」、エルドリンなら「私（わし）」、ケーレンなら「俺」）。\n\
            3. プレイヤーへの返答のセリフのみを日本語で出力してください。\n\
            4. 絶対に「ミナちゃん、〜」のように自分で自分の名前をちゃん付けで呼んで相手に語りかけるような、主客が逆転したおかしな日本語は禁止します。\n\
            5. 対話相手をお兄さん、お前、若者など、好感度と性格に合わせた二人称で呼んでください。\n\n\
            [過去の会話履歴]\n\
            {}\n\n\
            [プレイヤーからの話しかけ]\n\
            「{}」\n\n\
            [{}としての返答セリフ（地の文を少し含めても良い）]:",
            npc.name, npc.personality, npc.background, npc.current_emotion, npc.affection, npc.name, history_str, player_message, npc.name
        );
